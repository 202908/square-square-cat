...
export function updateSurvivalStats(account, seconds = 0) {
  ...
  nextAccount.health = clampPercent(nextAccount.health ?? 100);
  ...
  const died = nextAccount.hunger <= 0 || nextAccount.thirst <= 0 || nextAccount.health <= 0;
  if (died) {
    nextAccount.hunger = 100;
    nextAccount.thirst = 100;
    nextAccount.health = 100;
  }
  return { account: nextAccount, died };
}

export function damageAdultPlayer(account, amount = 18) {
  const nextAccount = structuredClone(account);
  nextAccount.health = clampPercent(nextAccount.health ?? 100);
  if (nextAccount.isHost || nextAccount.survivalMode !== "adult") {
    return { account: nextAccount, damaged: false, died: false };
  }
  nextAccount.health = clampPercent(nextAccount.health - Math.max(0, Number(amount || 0)));
  const died = nextAccount.health <= 0;
  if (died) {
    nextAccount.health = 100;
    nextAccount.hunger = 100;
    nextAccount.thirst = 100;
